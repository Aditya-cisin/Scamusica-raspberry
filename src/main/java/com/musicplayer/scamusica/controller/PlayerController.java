    ...
    public class PlayerController extends Application {
        ...
        private int countExistingInGenreFolder(String genreFolderPath) {
            ...
            int c = 0;
            for (File f : files) {
                if (!f.isDirectory() && f.getName().startsWith("song-") && f.getName().endsWith(".mp3") && f.length() > 0) {
                    c++;
                }
            }
            return c;
        ...
        private int countExistingDownloadedFiles(File root) {
            ...
            for (File f : files) {
                if (f.isDirectory()) {
                    ...
                    if (name.startsWith("song-") && name.endsWith(".mp3") && f.length() > 0) {
                        count++;
                    }
                }
            }
            ...
            try {
                ...
                if (downloadSeq.isEmpty()) {
                    ...
                    for (Integer id : downloadSeq) {
                        File candidate = new File(genreFolderPath, "song-" + id + ".mp3");
        ...
        private void playTrack(Label albumHeading,
            ...
            if (!safeUrl.contains(".mp3")) {
                safeUrl += "?ext=.mp3"; // Dummy parameter for Mac's AVFoundation
            }
            ...
            mediaPlayer.setOnError(() -> {
                ...
                System.out.println("Error Type: " + mediaPlayer.getError().getType());
            });

            media.setOnError(() -> {
                System.out.println("Media Source Error: " + media.getError().getMessage());
            });

            mediaPlayer.setOnReady(() -> {
                Duration mediaDuration = mediaPlayer.getMedia().getDuration();
                int durationSeconds = 1;
                if (mediaDuration != null && !mediaDuration.isUnknown()) {
                    durationSeconds = (int) Math.round(mediaDuration.toSeconds());
                }
